...
class CreateUser(TestUsers):
    ...
    def test_error_create_user(self):
        # unauthenticated => 401, no new user created
...
class GetUser(TestUsers):
    ...
    def test_get_user_detail(self):
        ...
        self.assertEqual(data['can_data_be_shared'], self.user.can_data_be_shared)
        self.assertEqual(data['can_be_contacted'], self.user.can_be_contacted)
        self.assertEqual(
            data['created_at'],
            self.format_datetime(self.user.created_at)
        )
...
class UpdateUser(TestUsers):
    def setUp(self):
        self.user = User.objects.create_user(
            username='alex', password='pass', age=30,
            can_data_be_shared=True, can_be_contacted=True
        )
        self.client.force_authenticate(user=self.user)
        self.url = reverse('user-detail', kwargs={'pk': self.user.pk})
    ...
    def test_put_user_full(self):
        new_data = {
            'username': 'alexandre',
            'password': 'password123',  # ou champ spécial selon votre serializer
            'age': 32,
            'can_data_be_shared': True,
            'can_be_contacted': True,
        }
...
class DeleteUser(TestUsers):
    ...
    def test_soft_delete_user(self):
        ...
        self.assertEqual(self.user.age, 99,
                         "age doit être mis à 99")
